...
- **`assembler/compounds.json` は UTF-8（BOMなし）・CRLF・「1行1件」・末尾に追記のみ。整形し直すの禁止**
  ...
  **`JSON.parse` → `JSON.stringify` で書き戻さないこと。** 直したい1件だけをその行の中で直す。
  ...
  - **過去3回破られた**（どれも「書き戻し」が原因。中身は無傷なので他の検査は全部通ってしまった）:
    （v1046・N-メチル基を1つ直しただけで 11516/11516）／
    （AI auto-save の便・209/10814）／
    （v1059・**SNS 動画のコミットの副作用**で 84890/917）
...
- **着手の前に課題台帳を引く**（`node tools/issues.js find <語>`）。**同じ件なら新しく作らず、その `I-####` で続ける**。
  会話の中で決めた仕事は台帳へ1行（`node tools/issues.js add …`）。直したらコミットの本文に id を書く
  （`git log --grep=I-0042` で前の試みが引ける）。⚠ 文書に「未着手・保留・あとで」と書くなら **id を同じ行に**
  —— `verify-release.js` の規則12 が止める。詳細は DEVELOPMENT.md「課題台帳」
- **セッションの始めに `node tools/triage-unmerged.js` を走らせる**（main に入っていない仕事の一覧・約7秒）。
  完成・検証済みの直しが統合されずに残り、あとで同じ作業がやり直されていた（2026-09-19 の調査で24本）。
  **新しく作業を始める前に、同じものが一覧に無いか見る**。捨てると決めたら `tools/branch-verdicts.jsonl` に1行。
  詳細は DEVELOPMENT.md「未統合の一覧」
