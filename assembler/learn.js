...
class IsomerPractice {
    ...
    parseFormula(str) {
        ...
        // ⚠ **Si はわざと載せない**（2026-09-23・I-0014）。ここを通ると異性体の数え上げに Si が
        //   4価の原子として入り、名前の出ない（iupacName が null の）異性体が並ぶ。
        //   高校の有機で Si の異性体・命名は扱わず、シリコーンの図は登録名から呼ぶので要らない
...
class ReferenceBook {
    ...
    /* いま再生している1件に表の上で印を付ける。
       ⚠ `reaction.js` の `syncList()` は 📚 学習の一覧（v1513 で撤去）を見ていたので、
