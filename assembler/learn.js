...
class IsomerPractice {
    constructor(game) {
         ...
         */
        /**
         * ★★ お題を選ぶ3本目の線（ユーザー明言 2026-09-01）:
         *   > **C7H14O2のエステルは人間には現実的ではなさそうです**
         *   > **ペンチル基に収まるところまででよいと思います**
         *
         * ★ **腕（種に付く R・R′）の炭素数が 5 まで** ＝ `functionalArmCarbons(mol).max <= 5`。
         *   ⚠ **「アルキル基」の数え方は2通りあり、実測すると結果が変わる**:
         *     読みA（**名前に置換基として現れる**アルキル基）… 落ちるのは **エステル C₇ だけ**
         *       （ギ酸ヘキシルの「ヘキシル」。ほかは全部メチル・エチル止まりで最大2）
         *     読みB（**種に付く腕** R・R′）… 落ちるのは **アルデヒド C₇・カルボン酸 C₇・エステル C₇**
         *       （どれも腕が6＝ヘキシル。ヘプタナールは C₆H₁₃–CHO で腕がヘキシル）
         *   ★ **採ったのは読みB**。理由は**4分類とも同じ1本の線になる**こと ——
         *     腕の最大は ケトン C₅→3/C₆→4/C₇→**5**、アルデヒド C₄→3/C₅→4/C₆→**5**、
         *     エステル C₃→2/C₄→3/C₅→4/C₆→**5**、カルボン酸 C₄→3/C₅→4/C₆→**5** で、
         *     **どの分類も「腕がヘキシルになる式」の1つ手前で止まる**。
         *     読みA は最大値が 1〜2 のところと 6 のところしか無く、**エステルにしか当たらない**
         *     ＝ 「ペンチル基に収まる」という言葉が線として機能しない。
         *   ⚠ **ただしこれは編集判断**。読みA を採るなら
         *     `{ formula:'C7H14O', cls:'aldehyde' }` と `{ formula:'C7H14O2', cls:'acid' }` を
         *     足すだけで開く（**名前は compounds.json に入れてある**）。
         *
         * ⚠⚠ **この線を `FG_ARM_MAX`（chemistry.js・列挙の門番）に下ろしてはいけない。**
         *   実測: 5 に下げるとエステル C₇H₁₄O₂ が **overflow で断られず、ギ酸ヘキシルだけ
         *   消えた 44種**になる ＝ 黙って数え落とす。詳しくは `functionalArmCarbons` の前書き。
         */
        this.fgPresets = [
            { formula: 'C5H10O',  cls: 'ketone' },     // 3種（腕 3）
            { formula: 'C6H12O',  cls: 'ketone' },     // 6種（★ユーザーが名指しした式・腕 4）
            { formula: 'C7H14O',  cls: 'ketone' },     // 15種（腕 5 ＝ ペンチルちょうど）
