 ...
 *
 * ⚠ ルール側の `reagentId` は**文字列でも文字列の配列でもよい**（v1427・同書 §12）。
...
/* 酸化剤の瓶は **KMnO₄ と K₂Cr₂O₇ の2本**（DESIGN_reagent_palette.md §12・v1427）。
 *
...
const REACTION_RULES = [
    ...
    {
        /* 1級アルコールを**一気に**カルボン酸まで（§12-2・v1427）。
         *
...
class Reactor {
     ...
     *
     * ⚠ v1427 で瓶 `oxidant` を KMnO₄ / K₂Cr₂O₇ の2本に割ったので、**古い id は別名で受ける**
     ...
     *    v1424 では濃硫酸（温度）にしか付いていなかったが、v1427 で酸化剤2本の
    ...
    runReagentHit(hit, reagent) {
        ...
        // ⚠ **押した瓶を持って行く**（v1427）。「効くが、ふつうはそちらを使わない」を
     ...
     * 要るのは濃硫酸の 160〜170℃／130〜140℃ と、酸化剤2本の 穏やかに／激しく（v1427）。
     *
     ...
     *
     * ⚠ **「ふつうはこちら」は瓶で変わる**（v1427・同書 §12-2）。行き先を決めるのは条件で、
    ...
    renderConditionChoice(reagent, options) {
        ...
        options.forEach(hit => {
            ...
            // 「この試薬ならふつうこちら」の印（v1427）。**押せる選択肢は両方とも同じ**
    ...
    explainConditionMiss(rule, pressed) {
        const note = this.reagentNoteEl;
        if (!note) return;
        // ⚠ **押された瓶を優先する**（v1427）。1つのルールが複数の瓶にぶら下がるようになったので、
    ...
    /**
     * 「**効くが、ふつうはそちらを使わない**」ときに結果へ添える一言（同書 §12-3・v1427）。
     *
    ...
    execute(rule, site, reagent) {
        ...
        /* 「効くが、ふつうはそちらを使わない」の一言を**結果に添える**（v1427・同書 §12-3）。
...
if (typeof window !== 'undefined') {
    ...
    // `reagentId` が文字列でも配列でもよいことを、テスト側も同じ関数で読む（v1427）
