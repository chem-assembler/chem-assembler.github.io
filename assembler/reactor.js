...
const REACTION_RULES = [
    ...
    },
    {
        ...
        apply(game, site) {
             ...
             * 「その分子を単独で描くときの形」へ移す** ＝ 前後の図が同じものを指していると読める。
             * ⚠ **「登録の向きにそろえる」のが目的ではない。** 登録の図は
             *   「単独で描くときの形」を取ってくる**手段**として引いているだけで、
             *   目的は上の1行（前後の対応）。ここを取り違えると、加水分解と切り離して
             *   呼ばれる「図の整形機能」に化ける。
             * ⚠ **必要があるときだけ動く。** マルトース・セロビオース・ラクトース・スクロースを
             *   そのまま切ったときは、生成物の形がもう単独のときと同じなので1件も動かない（実測）。
