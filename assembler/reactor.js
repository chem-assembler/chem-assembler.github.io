...
const REACTION_RULES = [
    ...
    },
    {
        ...
        apply(game, site) {
            ...
            const o = mol.addAtom('O', spot.x, spot.y);
            mol.addBond(cId, o.id, 1);
            /* ★ 切り出した単糖が、**単独で登録されている向き**と逆を向いていたら戻す（段4-c）。
             * ユーザーの発注「教科書通りのスクロースの図が、加水分解すると**フルクトース部分が
             * 反転し**、教科書通りのグルコースとフルクトースの図になる」がこれ。
             * ⚠ **必要があるときだけ動く。** 素のスクロース・マルトース・セロビオース・
             *   ラクトースからの加水分解では、生成物はもう登録の向きなので1件も戻さない（実測）。
             * ⚠ **分子の名前で分岐していない**（判定は「いまの向き ≠ 登録の向き」だけ）。
             *   前のレーンが「自動反転は不要」と結論したのは**名前で見た**ためで、
             *   名前は裏返しても絶対に変わらない ＝ 見るべきは向きだった。
             * ⚠ **加水分解のときだけ呼ぶ**（常時自動にすると、ユーザーが自分で裏返した絵を戻す）。 */
            const flips = game.restoreHaworthOrientation();
            const word = s => s === 1 ? '時計回り' : '反時計回り';
            const flipNote = flips.length
                ? flips.map(f => `なお、${f.name || 'できた単糖'}は二糖の中では逆向きに描かれていたので、` +
                    `単独で描くときの向き（炭素番号を${word(f.senseAfter)}にたどる向き）へ環を裏返して描き直しました。`).join('') +
                  '裏返すと「置換基の上下」と「炭素番号をたどる向き」が同時に入れ替わるので、分子も名前も変わりません。'
                : '';
            return {
                // ⚠ 戻した記録は**呼び出し側へ返す**（あとで反転をアニメーションにする人の材料）。
                //   `before` / `after` の座標が入っているので、そのまま補間できる
                haworthFlips: flips,
