...
const REACTION_RULES = [
    ...
    },
    {
        ...
        detect(mol) {
            ...
            /* ⚠⚠ **酸の側にも門番が要る**（v1502 の定期レビュー。もとは -COOH さえあれば
    ...
    },
    {
        ...
        detect(mol) {
            ...
            if (!units.length) return [];
            /* ③ ⚠⚠ **環の側にふつうの C=C / C≡C があったら出さない**（vNNNN。§10.14-G）。
    ...
    },
    {
        ...
        /* ⚠⚠ **門番は「その分子（連結成分）」だけを見る**（v1502 の定期レビューで直した）。
    ...
    },
    {
        ...
        detect(mol) {
            const groups = findFunctionalGroups(mol);
            const carboxyls = groups.filter(g => g.type === 'carboxyl');
            /* ⚠⚠ **-OH や -NH₂ を持つ酸を通してはいけない**（v1502 の定期レビューが実機で発見）。
