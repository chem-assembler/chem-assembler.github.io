...
/* ===== 名称呼び出しの候補の**並べ方**（v1526・ユーザー申し立て） =====
 *
 * ★ 実発生（原文ママ）:「呼び出し検索の順番は修正したい（アルファベット順 m,o,p より omp の
 *   方が見やすい，例えばベンゼンスルホン酸、といれたらベンゼンスルホン酸（無印）が一番上に
 *   くるのが望ましい」「初学者は、知らない化合物が上の方に並び、自分が出したい化合物が
 *   下の方にくるとやりづらいと思います」
 *   ⚠ v1525 までは**表示名の昇順で絞るだけ**だった（この形は v1406 の `<datalist>` 廃止より前から）。`o-` `m-` `p-` の付いた登録が 150件以上
 *   あるので、「ベンゼン」と打つと `m-クロロニトロベンゼン` から始まり、**探している
 *   「ベンゼン」は数十件下**にいた（統合レーンの実測）。
 *
 * ★ 物差しは4つ。**上から順に見て、違いが出たところで決着**する:
 *   ① 当たり方（tier）  0 完全一致 ／ 1 主名の前方一致 ／ 2 主名の部分一致 ／
 *                       3 **別名（括弧の中）だけに当たる**
 *   ② 本体の文字数（短い順）★ これがいちばん効く。**置換体の名前は必ず母体の名前を含んで
 *      長くなる**ので、短い順に並べるだけで母体が先・誘導体が後になる
 *   ③ 接頭辞の順位（下の表）★ `o-` → `m-` → `p-`。⛔ アルファベット順の m,o,p にしない
 *   ④ 表示名の**コード単位順**（★ 同点の完全な決着。`localeCompare` は環境で変わるので使わない
 *      ＝ 並びが実行のたびに変わると検査が不安定になる）
 *
 * ★ ②の「本体」＝ **括弧の別名を落とし、先頭の接頭辞も落とした部分**。
 *   ・別名を落とす理由 … `エチレン（エテン）` が別名を書いたぶんだけ沈むのはおかしい。
 *     物差しは「その化合物の名前の長さ」であって「別名を何字書いたか」ではない
 *   ・接頭辞を落とす理由 … `o-ジクロロベンゼン（オルト…）`／`m-…（メタ…）` は**別名の長さが
 *     1字違う**ので、表示名の長さで測ると o- が m- p- の後ろに落ちる ＝ ③が効かない。
 *     主名から接頭辞を外すと3つとも同じ長さになり、③で o→m→p に並ぶ
 *
 * ⚠ 接頭辞の判定は**大文字小文字を区別する**（`N-メチル…` の窒素置換と `n-ブチル…` の
 *   直鎖は別物）。照合（tier の判定）のほうは NFKC 正規化＋小文字化で、全角の `Ｏ-` も拾う。
 * ⚠ 表に無い接頭辞（`(1,1-ジメチル…` の括弧書きなど）は順位0＝「接頭辞なし」に落ちる。
 *   ②④ だけで決まるので並びは安定する（規則が単純なほうが、なぜこの順かが読者に分かる）。
 *   位置番号（`1-` `2,3-` など）はまとめて 70 で、その中は④の名前順 ＝ 1- → 2- → 3-。
 *
 * ⚠⚠ **採らなかった案**: 一問一答の難易度（Lv1〜4）や入試DBの出題頻度で並べる。
 *    ユーザーと合意した順序は「**まず上の3規則で作り、それでも沈むものがあれば頻度を足す**」。
 *    ＝ 頻度は**次の一手**であって今回は入れない。3規則は登録データだけで決まり、
 *    他のアプリのデータに依存しないので、ここが壊れる筋が少ない。
 */
const SUMMON_PREFIX_RANK = [
    ['o-', 10], ['m-', 11], ['p-', 12],                      // ベンゼン環の位置（★ o→m→p）
    ['シス-', 20], ['cis-', 20], ['トランス-', 21], ['trans-', 21],
    ['α-', 30], ['β-', 31], ['γ-', 32], ['δ-', 33], ['ε-', 34],
    ['D-', 40], ['L-', 41],                                  // 糖・アミノ酸の系列
    ['n-', 50], ['sec-', 51], ['tert-', 52], ['iso-', 53],   // ⚠ `n-` は小文字（`N-` と別物）
    ['N,N-', 61], ['N-', 60]                                 // ⚠ 長いほうを先に見る
];
/** 照合用の正規化。全角の `Ｏ-` を `o-` に、大文字小文字の差を消す */
const summonNorm = (s) => String(s).normalize('NFKC').toLowerCase();
/** 表示名 → 並べ替えの物差し（上のコメント②③④）。1名につき1度だけ作って使い回す */
function summonNameKey(name) {
    const 主名 = name.replace(/（.*$/, '');       // 括弧の別名から後ろは物差しに入れない
    let 順位 = 0, 本体 = 主名;
    for (const [p, r] of SUMMON_PREFIX_RANK) {
        if (主名.startsWith(p)) { 順位 = r; 本体 = 主名.slice(p.length); break; }
    }
    if (順位 === 0) {
        const m = 主名.match(/^\d+(?:,\d+)*-/);   // 位置番号（1- / 2,3- / 2,4,6- …）
        if (m) { 順位 = 70; 本体 = 主名.slice(m[0].length); }
    }
    return { name, 主名: summonNorm(主名), 全体: summonNorm(name), 本体長: [...本体].length, 順位 };
}
...
// 🔢 主鎖と番号で「アルキル基」を塗る2色（エーテルの両側・エステルのアルコール側が共有。DESIGN_iupac_check.md §N-5・§13）
const IUPAC_GROUP_COLORS = ['var(--neon-orange, #ffa502)', 'var(--neon-pink, #ff2a85)'];
...
class Game {
    ...
    /**
     * 打った文字列 `q` に当たる候補を、**見つけやすい順**に並べて返す（v1526）。
     *
     * ★ 順序の規則そのものは `SUMMON_PREFIX_RANK` / `summonNameKey` の上のコメントが正。
     * ★ **並べ方を決めるのはここ1箇所**。入力欄は2つ（帯・モーダル）あるが、どちらも
     *   `attachSummonCombo` を通ってこの関数を呼ぶ ＝ 2か所に書かない（SM6 ⑥ が見張る）。
     * ⚠ `q` が空のとき（欄に触っただけ）も**同じ物差し**で並べる。tier は全部0になるので
     *   「本体が短い順」＝ 水・ギ酸・酢酸・メタン… と、いちばん基本的なものが上に来る
     *   （v1525 までは表示名の昇順で、`(1,1,2-トリメチルプロピル)アミン` が先頭だった）。
     */
    summonCandidates(q) {
        const names = this.summonNames || [];
        if (!this._summonKeys || this._summonKeys.size !== names.length) {
            this._summonKeys = new Map(names.map(n => [n, summonNameKey(n)]));
        }
        const 語 = summonNorm(String(q == null ? '' : q).trim());
        const 当たり = [];
        for (const n of names) {
            const k = this._summonKeys.get(n);
            if (!k) continue;
            let tier = 0;
            if (語) {
                if (k.全体.indexOf(語) < 0) continue;
                tier = (k.全体 === 語 || k.主名 === 語) ? 0
                    : k.主名.startsWith(語) ? 1
        ...
        当たり.sort((x, y) => (x.tier - y.tier)
            || (x.k.本体長 - y.k.本体長)
            || (x.k.順位 - y.k.順位)
            || (x.k.name < y.k.name ? -1 : x.k.name > y.k.name ? 1 : 0));
        return 当たり.map(e => e.k.name);
    }
    ...
    attachSummonCombo(input, commit) {
        // ⚠ 候補の名前は**掴まない**（v1526）。絞り込みと並べ直しは `summonCandidates` に任せる
        //    ＝ 入力欄が2つあっても並べ方は1箇所。ここで配列を写しに取ると、片方だけが
        //    古い並びを持ち続ける道ができる
        ...
        /**
         * 絞り込みと並べ直し。
         * ・当て方 … **部分一致**（`<datalist>` のときと同じ。「ブタノール」で 1-/2- が出る）
         * ・並び順 … `summonCandidates`（★ v1526 で変えた。規則は `summonNameKey` の上）
         *
         * ⚠ v1406〜v1525 はここに「**`<datalist>` のときと同じにする（勝手に変えない）**：
         *    並び順は `summonNames` の並び（表示名の昇順）のまま」と書いてあった。
         *    ★ **なぜ変えたか** ―― ユーザーの申し立て（v1525・原文ママ）
         *      「アルファベット順 m,o,p より omp の方が見やすい」
         *      「ベンゼンスルホン酸、といれたらベンゼンスルホン酸（無印）が一番上にくるのが望ましい」
         *      「初学者は、知らない化合物が上の方に並び、自分が出したい化合物が下の方にくると
         *        やりづらいと思います」。
         *    昇順のままだと `o-` `m-` `p-` の付いた登録（150件以上）が先に来て、**母体の名前ほど
         *    下に沈む**。「勝手に変えない」は**ブラウザの挙動に合わせるための約束**であって、
         *    ユーザーが見づらいと言ったものを守る約束ではないので、ここで解いた。
         *    ⚠ ただし**当て方（部分一致）は変えていない** ＝ 出る顔ぶれは1件も変わらず、順番だけ変わる
         */
        const 並べる = () => {
            const q = input.value.trim();
            const 当たり = this.summonCandidates(q);
            box.innerHTML = '';
    ...
    _iupacDetailFor(mol) {
        ...
        // ⚠ `groups` はエステルのアルコール側（1つ。v1582・§13）。エーテルと同じ形で、帯の2色目と基の名前になる
    ...
    _iupacDrawOne(det, hidden) {
        ...
        if (det.kind === 'ether') {
            ...
            det.groups.forEach((g, i) => this._iupacGroupBand(this.userMolecule, g, IUPAC_GROUP_COLORS[i], byId, visible, this.bondsGroup, this.atomsGroup));
            lines.push(`🔢 ${det.name}`);
            ...
            // ★ エステルのアルコール側（v1582・§13）: 番号は酸の部分だけ。O についたアルキル基は
    ...
    /**
     * アルキル基1つを帯（色）と基の名前で示す（エーテルの両側・エステルのアルコール側が共有。v1582 で切り出し）。
    ...
    _iupacGroupBand(mol, g, color, byId, visible, bondsTarget, textTarget) {
        ...
        mol.bonds.forEach(b => {
            ...
            this._iupacBand(byId.get(b.atomId1), byId.get(b.atomId2), color, bondsTarget);
        });
        if (g.ids.length === 1 && visible(g.rootId)) {
            const a = byId.get(g.rootId);
            if (a) this._iupacBand({ x: a.x - 7, y: a.y }, { x: a.x + 7, y: a.y }, color, bondsTarget);
        }
        ...
        if (pts.length) {
            ...
            this._iupacText(cx, cy - 26, g.name, color, 11, textTarget, 'iupac-group-name');
        }
    }
    ...
    iupacPartAtoms(det, part) {
        ...
        // エステルのアルコール側（v1582）: `groups[0].ids` ＝ O についたアルキル基の炭素
        ...
        if (part.kind === 'one' || part.kind === 'al' || part.kind === 'oic' || part.kind === 'oate' || part.kind === 'dioic') {
            // カルボニル（v1576）。位置番号（`2-`）は**その炭素**、接尾辞（`ノン`）は**その =O**。
            // アルデヒド・カルボン酸は位置番号のかけらが無いので、接尾辞が C1 と O をまとめて指す
            // （-CHO・-COOH は「1番の炭素がその基になっている」と読む）。
            // エステルの「酸」は C1 と =O と −O−（v1582）、二酸の「二酸」は両端（locs=[1,n]）をまとめて指す
            locs.forEach(loc => {
                const cid = at(loc);
                if (cid == null) return;
    ...
    iupacDirectionReason(det) {
        ...
        switch (det.dirReason) {
            case 'carbonyl': {
                ...
                // エステル（v1582）: 番号は酸の部分だけ。O についたアルキル基は数えない
            ...
            case 'tie':
                // 二酸（v1582）: 両端が −COOH なので向きは決まらない ＝ それを言う
    ...
    _iupacPartNote(det, part, parts, i) {
        ...
        switch (part.role) {
            ...
            case 'ester-alkyl': return `エステルのアルコール側のアルキル基「${part.label}」です（O についていて、番号は数えません）。`;
            default: return '';
        }
    }
    ...
    _iupacUnitNote(det, parts, group) {
        ...
        if (ps.length === 1 && ps[0].role === 'suffix' && ps[0].kind === 'dioic') {
            return `${where(ps[0].locs || [])}の炭素がカルボキシ基（−COOH）になっているね（両端なので位置番号は書かない）。`;
        }
        if (ps.length === 1 && ps[0].role === 'ester-alkyl') return `O についた${ps[0].label}基がアルコール側だね。`;
    ...
    /**
     * エステルの酸の部分の名前（系統名。慣用名があれば括弧で添える）。v1582・§13。
     ...
     * 引けなければ系統名だけ
     */
    _iupacEsterAcidName(det, parts) {
        const k = parts.findIndex(p => p.role === 'ester-alkyl');
        if (k < 0) return '';
        const sys = parts.slice(0, k).map(p => p.text).join('');
        const alkyl = parts[k].text;
        const lib = this.lookupCompoundName(det.mol || this.userMolecule) || '';
        const cands = [lib.replace(/（.*$/, '')].concat((lib.match(/（([^）]*)）/g) || []).map(s => s.slice(1, -1)));
        const hit = cands.find(s => s.endsWith(alkyl) && s.length > alkyl.length);
        const triv = hit ? hit.slice(0, -alkyl.length) : '';
        return (triv && triv !== sys && triv.indexOf(sys) < 0) ? `${sys}（${triv}）` : sys;
    }
    ...
    _iupacGlow(det, hidden) {
        ...
        const framed = roles.some(p => p.role === 'sub' || p.role === 'ether-group' || p.role === 'ester-alkyl' || p.kind === 'ol' ||
            p.kind === 'one' || p.kind === 'al' || p.kind === 'oic' || p.kind === 'oate' || p.kind === 'dioic');
        if (!framed) return;
    ...
    drawIupacNumberingIntoSvg(svgId, target) {
        ...
        if (d.kind === 'ether') {
            ...
            d.groups.forEach((grp, i) => this._iupacGroupBand(mol, grp, IUPAC_GROUP_COLORS[i], byId, all, bonds, atoms));
            this._iupacLiftBondInk(bonds);   // 帯の上でも 2本線が読めるように（C-2）
            return true;
        }
        if (d.kind !== 'chain' || !d.mainChain || !d.mainChain.length) return false;
        const chain = d.mainChain.map(id => byId.get(id)).filter(Boolean);
        // エステルのアルコール側（v1582）: キャンバスと同じ2色目の帯と基の名前
...
const OPEN_TARGETS = {
    ...
    /* ⚠⚠ v1513 で 📚 学習の ⚗️ 反応機構ビューアの札を撤去した（ユーザー決定 2026-09-03）。
...
function applyOpenParam(search) {
    ...
    /* 受け口④ `?open=mechanism&id=<機構id>` … 登録済みの機構から1つを選んで再生する。
       ...
       ⚠ **`reactionPlayer.openById` を直に呼ばず `referenceBook.playMechanism` を通す**（v1513）——
