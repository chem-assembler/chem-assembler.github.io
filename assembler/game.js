...
class Game {
    ...
    saveState() {
        ...
        /* ★ **変化の直前の「見ている分子」を控える**（vNNNN・`refocusToMainFragment` が読む）。
    ...
    updateDrawing() {
        ...
        // ★ 「いま見ている分子」が割れていたら、焦点を大きいほうの破片へ移す（vNNNN）。
    ...
    /**
     * ★ **「いま見ている分子」が割れたら、焦点を大きいほうの破片へ移す**（vNNNN）。
     *
     ...
     *
     * ⚠ **「焦点を外す（キャンバス全体に戻す）」では直らない**（実測・vNNNN）:
    ...
    renderFocusFrame(hidden) {
         ...
         *
         * ★★ **例外は 🧪 実験パレットのときだけ**（vNNNN・ユーザー実機報告 2026-09-03
    ...
    fitCanvasToMolecule(targetMolecule) {
        ...
        if (rect && rect.width > 0 && rect.height > 0) {
            ...
            /* ★★ **狭い画面では、余白と最小視野を「分子が入る限界」まで詰める**（vNNNN）。
             *
