...
(() => {
    ...
    test('PY1: 高分子は総数当てに出さない（判定は「ポリ」の名前でなく擬似元素 R で行う・v1020）', async (c) => {
        ...
        q.pool = null;
        q.build();
        const after = q.pool.length;
        const poly = q.pool.filter(p => hasR(p.mol));
        assert(poly.length === 0,
            `高分子が出題プールに入っている（${poly.map(p => p.name).join('・')}）`);
        assert(!q.pool.some(p => p.name === 'ポリアセチレン' || p.name === 'ポリビニルアルコール'),
        ...
        try {
            ...
            q.pool = null;
            q.build();
            const back = q.pool.filter(p => hasR(p.mol)).map(p => `${p.name}=${p.count}`);
            assert(back.length === 2 && back.includes('ポリアセチレン=6') && back.includes('ポリビニルアルコール=8'),
                `否定対照が成立しない（絞り込みを外しても高分子が戻らない: ${back.join('・') || 'なし'}）`);
            assert(q.pool.length === after + 2,
                `絞り込みで減る件数が2でない（${q.pool.length} → ${after}）`);
        } finally {
