 ...
 * | D   | 1〜6   | 結合の伸縮・側鎖の向き |
 * | DG  | 1〜2   | 辞書引き（📚 同じ分子式の異性体を調べる）の門番と、数えない代わりに出す在庫 |
...
(() => {
    ...
    // ===== DG: 辞書引き（📚 同じ分子式の異性体を調べる）の門番（DEVELOPMENT.md §7-1h） =====
    // 練習側（IS1）と**帯は同じ・断り方は別**。ここは分子式を選ぶ画面ではなく、
    // 目の前の分子について聞く辞書引きなので、「別の式で試して」とは言えない

    // 名称から1分子だけ呼び直すヘルパー（前の分子が残ると countMolecules() > 1 で門前払いになる）
