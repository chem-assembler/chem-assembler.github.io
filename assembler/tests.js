 ...
 *
 * ===== 使用済みのテスト番号（**新しいテストを足す前にここを見る**） =====
 *
 * 並行レーンで同じ番号を別々に使ってしまい、`F9`・`F10`・`ST37` が2回ずつ定義された
 * ことがある（どちらが落ちたのか名前で特定できない ＝ 回帰テストの意味が薄れる）。
 * **着手時に見る場所をここ1つにする**ための一覧。番号を使ったらこの表を伸ばすこと。
 * 一意であることは `IS2` が機械で見張っている（重複が1件でもあれば赤くなる）。
 *
 * | 接頭辞 | 使用済み | 守備範囲 |
 * |---|---|---|
...
(() => {
    ...
    test('F12: 「同じ化合物？」クイズが出題の前提を明示する（立体の種類を取り違えない）', async (c) => {
    ...
    // テスト台帳の自己点検。番号が重複していると「どちらが落ちたのか」を名前で特定できず、
    // 失敗の一覧が読めなくなる（F9・F10・ST37 が実際に2回ずつ定義されていた）。
    // 冒頭の一覧（使用済みの接頭辞と番号帯）が守られているかを機械で見張る
    test('IS2: テスト名が全件で一意（番号の衝突を機械で見張る）', async () => {
        const dupsOf = (list) => {
            const count = new Map();
            list.forEach(n => count.set(n, (count.get(n) || 0) + 1));
            return [...count.entries()].filter(([, k]) => k > 1).map(([n, k]) => `${n}×${k}`);
        };
        assert(tests.length >= 200, `テストの登録数が少なすぎる（${tests.length}件）。台帳を数える前提が崩れている`);

        // (a) 番号（「:」の前）が全件で一意
        const ids = tests.map(t => String(t.name).split(':')[0].trim());
        const idDups = dupsOf(ids);
        assert(idDups.length === 0,
            `テスト番号が重複している: ${idDups.join(' / ')}。tests.js 冒頭の一覧を見て空き番号に振り直すこと`);

        // (b) 名前まるごとでも一意（番号が違っても説明文が同じなら、どちらを直すのか分からない）
        const fullDups = dupsOf(tests.map(t => String(t.name)));
        assert(fullDups.length === 0, `テスト名が重複している: ${fullDups.join(' / ')}`);

        // (c) 否定対照 —— わざと重複させた配列では、この見張りが赤くなること
    ...
    test('ST30: R・S の読み物（用語と決め方の骨組み＋実装との一致・M2.5 その3）', async (c) => {
        ...
        // **この主張が本当かは ST42 がアプリ自身の判定で確かめる**ので、
        // ここは「書いてあるか」だけを見る
        ...
        //     約束どおり記号が出ていなければならない。出ない図の扱いは ST42 が見る
    ...
    test('ST18: 立体異性体の総数当てクイズ（M2.5 出題）', async (c) => {
        ...
        // 手に負えない大きさの題材を出さない（v980）。二糖は 2¹⁰=1024 で、しかも
        // 畳み込みが起きない ＝ このクイズの主眼「2ⁿ が崩れる」に寄与しないので外した
        assert(q.pool.every(p => p.naive <= 32),
            `2⁵=32 を超える出題が混じっている（${(q.pool.find(p => p.naive > 32) || {}).name}）`);
        assert(!q.pool.some(p => /マルトース|ラクトース|セロビオース|スクロース/.test(p.name)),
            '二糖が出題プールに入っている（答えが 1024 / 512 になる）');
        // 高分子を出さない（v982）。末端を R で止めた有限の図から出る「6種類」「8種類」は
        // 描いた単位の個数で変わる人工物で、実物の立体規則性（iso/syndio/atactic）とは別物。
        // とくにポリアセチレンは畳み込み(8→6)が起きるため、重み付き出題で優先されてしまっていた
        assert(q.pool.every(p => !p.mol.atoms.some(a => a.element === 'R')),
            `高分子（R で止めた繰り返し単位）が出題プールに入っている（${(q.pool.find(p => p.mol.atoms.some(a => a.element === 'R')) || {}).name}）`);
        assert(!q.pool.some(p => /ポリアセチレン|ポリビニルアルコール/.test(p.name)),
            'ポリアセチレン・ポリビニルアルコールが出題プールに入っている');
        // 糖が丸ごと消えたわけではないことも固定する（入試定番の「アルドヘキソースは16種類」）
        assert(q.pool.some(p => p.name === 'D-グルコース（鎖状）' && p.count === 16),
            '鎖状 D-グルコース（2⁴=16）が出題プールから落ちている');
        ...
        // **2ⁿ が崩れる理由は2通りある**（メソ体／環の回転対称）。設計（M2.5 その4）は
        // その両方を体験させることを主眼にしているのに、v980 まで**環の回転対称の題材が
        // 出題プールに1件も無かった**（ST16 のテスト内と ST30 の書き出し練習にしか無かった）。
        // v981 で compounds.json に収録して塞いだ。ここが落ちたら主眼の半分が失われている
        assert(folded.some(p => p.name === '乳酸3分子の環状エステル' && p.naive === 8 && p.count === 4),
            '乳酸3分子の環状エステルが 2³=8 → 4 として入っていない（環の回転対称の題材）');
    ...
    test('ST42: R・S を画面に出す（判定できない図では理由を出す・v570）', async (c) => {
        c.reset();
        const W = c.W, D = c.D, g = c.game;
        const sv = W.stereoView;
        assert(sv, 'stereoView が初期化されていない');
