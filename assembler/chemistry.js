...
function enumerateConstitutionalIsomers(elements, hCount, nodeLimit = 600000) {
     ...
     * **答えの集合は1つも変わらない**（重原子6個までの全804式で確認済み）。
     *
     * ★ **それでも入れてはいけない理由**: 同型類から**どの代表を残すか**が変わる。
     * `seen` は最初に出会った1つを残すので、**列挙が返す `Molecule` の形（原子の並び・
     * どの添字が繋がっているか）が総取り替えになる**。これは下流に効く:
     *   ・答え合わせの正解図の**自動レイアウト**が変わる（`IW31`「エーテルの正解図は
     *     主鎖が横一直線」の否定対照が空振りになった）
     *   ・`findAnyCycle` が返す環が変わる（`NW3b`「C₆H₆ の六員環は33通り」が5通りに落ちた）
     *   ・凍結リストとの突き合わせが外れる（`IN2` / `IN12`）
     * ＝ **速さのために、画面に出る図を黙って全部差し替える**ことになる。
     * 速さが要るなら、代表を変えない形（下の `treeOnly` のように、
     * **record() が捨てる枝だけを前倒しで捨てる**）で取ること。
     */
...
/* 倍数接頭辞（同じ置換基が何個あるか）。
 * ⚠⚠ **v1511 まで 8 で切れていた。** 表に無い個数では `IUPAC_MULT[n] || ''` が**黙って接頭辞を
 *   落とし**、`1,1,1,2,2,3,3,4,4-クロロブタン` のような**誤った名前**が画面に出ていた
 *   （実測。ノナ が消えている）。★ アルカンの光塩素化（v1511）を入れて 9 個目に手が届くように
 *   なったので直した。⚠ 手で描けば前から出せたので、**新しく生まれたバグではない**。
 * ★ 直しは2つ。**両方要る**:
 *   ① 表を 20 まで伸ばす（在庫のアルカンを塩素で埋めても届く範囲）
 *   ② 表に無い個数では**名前を作らない**（`iupacName` が null を返す）＝ 言い切れないものは
 *      名乗らない、というこのリポジトリの約束。★ ①だけだと 21 個目でまた黙って嘘をつく */
const IUPAC_MULT = { 1: '', 2: 'ジ', 3: 'トリ', 4: 'テトラ', 5: 'ペンタ', 6: 'ヘキサ', 7: 'ヘプタ', 8: 'オクタ',
    9: 'ノナ', 10: 'デカ', 11: 'ウンデカ', 12: 'ドデカ', 13: 'トリデカ', 14: 'テトラデカ',
    15: 'ペンタデカ', 16: 'ヘキサデカ', 17: 'ヘプタデカ', 18: 'オクタデカ', 19: 'ノナデカ', 20: 'イコサ' };
...
function _iupacAssemble(stem, subs, omitLocants, out) {
    ...
    // 倍数接頭辞の表に無い個数（21個以上）＝ 言い切れないので名乗らない。
    // ⚠ ここが無いと `IUPAC_MULT[n] || ''` が接頭辞を黙って落として**誤った名前**になる
    if (groups.some(g => IUPAC_MULT[g.locs.length] === undefined)) return null;
...
// ★ v1582（DESIGN_iupac_check.md §13）:
...
function _iupacCarbonylCore(n, kind, loc, out) {
    ...
    const push = (text, role, extra) => { if (out) out.push(Object.assign({ text, role }, extra || {})); };
    if (kind === 'one') {
        const tail = { kind: 'one', locs: [loc] };
        const omit = n === 3;   // プロパノンは C2 しか C=O になれない
        if (!omit) push(loc + '-', 'locant', tail);
        push(stem, 'stem', { size: n });
        push('ノン', 'suffix', tail);
        return (omit ? '' : loc + '-') + stem + 'ノン';
    }
    if (kind === 'al') {
        push(stem, 'stem', { size: n });
 ...
 *               //   ★ エステル（v1582）は kind==='chain' のまま**1つ**（アルコール側のアルキル基。oId ＝ −O−）。他は null
 ...
 * ★ エステル（酸の名前＋アルキル基。`エタン酸エチル`）と二酸（`ブタン二酸`）は v1582（§13）で足した。
 ...
 *     'ester-alkyl'  … エステルのアルコール側のアルキル基（`エチル`／`(1,1-ジメチルプロピル)`。v1582）。`groups`=[0]・`label`
 *     'locant'       … 位置番号（`-1-`）。`kind` ＝ 'ol'|'ene'|'yne'|'one'・`locs`
 *     'stem'         … 幹（`プロパ`）。`size` ＝ 主鎖の炭素数
 *     'suffix'       … 接尾辞（`ノール`・`ノン`・`ナール`・`酸`）。`kind`（'ol'|'ene'|'yne'|'one'|'al'|'oic'）・
 *                      `locs` は locant と同じ意味（al/oic は位置番号を書かないので locs=[1] の suffix だけ）
 *     'sat'          … 飽和の印（アルカンの `ン`）
...
function iupacNameDetail(mol) {
    ...
    //   カルボン酸（末端の -OH が1つ）／エステル（-O- の先に炭素1個。v1582・§13）に分ける。
    ...
    for (const o of oxygens) {
        ...
        const c = nb[0].atom;
        const cn = mol.getNeighbors(c.id).filter(n => n.atom.id !== o.id && n.atom.element !== 'H');
    ...
    if (mol.bonds.some(b => b.type >= 2 && (!carbonIds.has(b.atomId1) || !carbonIds.has(b.atomId2)) &&
        !carbonyl.some(x => (b.atomId1 === x.cId && b.atomId2 === x.oId) || (b.atomId2 === x.cId && b.atomId1 === x.oId)))) return null;
    ...
    // 酸素の分類: -OH（炭素1個・単結合）か エーテル（炭素2個・単結合）。それ以外（ペルオキシド等）は未対応
    const hydroxylC = [];
    let etherCount = 0;
    for (const o of oxygens) {
        if (coOxygen.has(o.id)) continue;   // C=O とカルボン酸の -OH は上で数えた
    ...
    //   位置番号の省略規則が要るので名乗らない。
    // ★ v1582（§13）: C=O が2つのときは**両方がカルボン酸（-COOH）のとき＝二酸**だけ名乗る（`kind:'dioic'`）。
    ...
    // ★ エステル（v1582・§13）: 炭素は −O− をはさんで**酸の部分**と**アルコールの部分（アルキル基）**の
    ...
    if (chainCarbons.length === 1) {
        ...
        if (bestOh < prinSet.size) return null;          // 全OHを1本の鎖に収められない（分岐ポリオール・未対応）
    ...
    named.sort((a, b) => _iupacCmpLocants(a.coLocs, b.coLocs) || _iupacCmpLocants(a.olLocs, b.olLocs) ||
        _iupacCmpLocants(a.unsat, b.unsat) ||
    ...
    const best = named[0];
    // エステルのアルコール側（v1582）。エーテルの `groups` と同じ形で1つだけ（画面は2色目の帯と基の名前で示す）
...
function _iupacNameForMainChain(adj, haloAdj, cbond, chain, ohSet, co) {
    ...
    const coreParts = [];
    const core = co ? _iupacCarbonylCore(n, co.kind, d.coLocs[0], coreParts)
    ...
    if (!name) return null;
    // ★ エステル（v1582・§13）: 酸の名前の後ろに**アルコール側のアルキル基**を足す（エタン酸＋エチル）。
    ...
    return {
        ...
        olLocs: oL, eneLocs: eL, yneLocs: yL, coLocs: d.coLocs, unsat: _iupacSortNum(eL.concat(yL)), locants: subLocs(d)
    };
}
