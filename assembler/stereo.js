...
//
// ⚠ v1442 で断り文を直した。**二糖は縮合環ではない**（-O- 1本でつないだ独立な2つの環）のに、
...
const RING_COPLANAR_CAVEAT =
    '⚠ この図は「2つの環が同じ平面にある」と決めて描いています（そう仮定して見ている図です）。' +
...
class StereoView {
    ...
    buildRingModel() {
        ...
        // ⚠ **橋1本でつないだ環2つ（二糖）はここを通す**（v1442。それまでは縮合環と同じ文で断っていた）
        ...
        /* ★★ 橋の -O- は**2つの host 環原子のちょうど中間**に置く（v1467・DESIGN_3d_correspondence.md §7.1b）。
         *
         * **ユーザー報告（2026-08-26・実機）**:
         * > **「ハース環　2糖　立体視　環をヨコから → グリコシド結合が、結合炭素から
         * >   ずれた位置に斜めに伸びている」**
         *
         * ★ **実測した原因**（推測ではない）: 上の枝づくりでは橋だけ
         *   **描かれた 2D 座標（x も y も）**を使っていた（§7.1a の「横位置は描かれたまま」）。
         *   ところが**模型の y は環の面の中**なので、キャンバスで橋の -O- が host の
         *   **114px 下**に描かれている（面を ±25° 以内で読ませるために必要な深い V。§11）と、
         *   模型でも面の中を 114px ぶん流れる。実測（登録の二糖4件）:
         *
         *     直す前 … host からの**面内**のずれ 115〜129 ／ 枝の長さ 127〜139 ／ 法線から **65〜67°**
         *     ほかの枝 … 面内のずれ **0** ／ 長さ 53.7（＝ depth）／ 法線から **0°**
         *
         *   ＝ 橋だけが「host の真下ではないところへ、寝た向きに」伸びていた。これが報告の絵。
         *
         * ★ **直し方**: 面内の位置を**両側の host の中点**にし、高さ（z ＝ face×depth）は据え置く。
         *   ⚠ **橋を法線に立てることは原理的にできない**（2つの環の置換基を兼ねるので
         *   両方には垂直に立てられない。§7.1a）。できるのは
         *   **「面内のずれを最小にし、両側で対称にする」**ことで、中点がちょうどそれ。
         *
         *     直したあと … 面内のずれ **42**（＝ 2つの環の間隔の半分）／ 長さ 68.2 ／ 法線から **38°**
         *
         * ⚠ **face（上下）は1つも変えていない** ＝ α/β の読みにも命名にも触らない
         *   （変えるのは v の面内成分だけ）。RP1 ③ の「橋の高さが他の置換基とそろっている」も無傷。 */
        if (bridgeNodeIdx >= 0 && bridge) {
            const hostNode = (id) => nodes.find(n => n.kind === 'ring' && n.atomId === id);
            const hA = hostNode(bridge.hostA.id), hB = hostNode(bridge.hostB.id);
            if (hA && hB) {
                const bn = nodes[bridgeNodeIdx];
                bn.v = [(hA.v[0] + hB.v[0]) / 2, (hA.v[1] + hB.v[1]) / 2, bn.v[2]];
            }
        }

        // 二糖で環の大きさが同じだと「六員環と六員環」になって、どちらの話か分からなくなる。
        // 置かれた位置（左右／上下）で呼び分ける（セロビオース・ラクトース・マルトース）
        if (rings.length === 2 && rings[0].size === rings[1].size) {
            const mid = rings.map(r => {
                const p = [];
                for (let i = 0; i < r.size; i++) p.push(nodes[r.base + i].v);
                return {
            ...
            const wordFor = i => byX
                ? (mid[i].x <= mid[1 - i].x ? '左' : '右')
                : (mid[i].y <= mid[1 - i].y ? '上' : '下');
            rings.forEach((r, i) => { r.label = wordFor(i) + 'の' + r.label; });
        }
    ...
    updateRingNote() {
        ...
        if (m.bridge) {
            ...
            if (m.bridge.flipped) {
                parts.push(`★ ${b.label}のほうは裏返して置いています。こうしないと、橋の酸素の上下が` +
                           '2つの環で食い違って「同じ平面」に置けません。' +
                           '裏返すと「置換基の上下」と「番号をたどる向き」が同時に入れ替わるので、' +
                           '分子は同じままです（アプリが出す名前も変わりません）。' +
                           '⚠ 片方だけ ——たとえば上下だけを入れ替える—— と別の分子（鏡像体）になります。');
                // 教科書のどの絵の話なのかを、環の大きさで言い分ける
                // （五員環が裏返る ＝ スクロースのフルクトース／六員環どうし ＝ β-1,4 のセルロース）
                parts.push(b.size === 5
                    ? '教科書のスクロースでフルクトース環が「ふつうと逆向き（反時計回り）」に' +
                      '描かれているのは、この裏返しのためです。'
                    : 'セルロースの図で環が1つおきにひっくり返って描かれるのも、これと同じ裏返しです' +
                      '（β-1,4 でつなぐと橋の酸素の上下がそろわないため）。');
