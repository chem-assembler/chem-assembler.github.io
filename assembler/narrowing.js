...
//
// ⚠ **以前は NARROW_CARDS の後ろに push で足していた**が、そうすると
// カードの定義順＝行の並びの末尾に「環の大きさ」が来て、**「環」の行から9行離れる**。
// 行の並びはカードの定義順で決まる（下の NARROW_ROW_ORDER）ので、
// **並べたい場所に定義を置く**のが唯一の並び替え手段。ここで先に作って環の直後に差し込む。
const RING_KANJI = { 3: '三', 4: '四', 5: '五', 6: '六', 7: '七', 8: '八' };
const RING_SIZE_CARDS = [3, 4, 5, 6, 7, 8].map((n) => ({
...
const NARROW_CARDS = [
    ...
    // 環の大きさ（上で組み立てたもの）。**「環」の行のすぐ下に出したい**のでここに差し込む
    ...RING_SIZE_CARDS,
    ...
    /**
     * ★★ M12: **加水分解してから、出てきた片割れの性質を見る**（v1505・ユーザー要望 2026-08-31
...
// 行の並び。**カードの並び順そのまま**（別に台帳を持つと、
// カードを足したのに行が出ない／空の行が出る、という食い違いが起きる）。
const NARROW_ROW_ORDER = [...new Set(NARROW_CARDS.map((c) => c.row))];

/**
 * マトリクス（M2）の行の台帳。**パレットと同じもの**を使う。
 *
 * ⚠ **以前は手書きの16行だった**。上の警告を書いた本人がもう片方で踏んでいて、
 * 後から足した **芳香環・環上の位置・アミノ基・アミド の4行が抜けたまま**になっていた
 * ＝ ベンゼン環・ニンヒドリン・アミドのカードを積むと、候補はちゃんと絞られるのに
 * **表には何も出ない**（窒素を含む問題はまるごと表に映らなかった）。
 *
 * **4行を足すのではなく手書きの台帳ごと捨てた。** 足すだけでは次にカードを増やした人が
 * また忘れる ＝ 同じ事故が起き続ける。
 *
 * ⚠ **行の並びを変えたくなったら、ここではなくカードの定義順を動かすこと。**
 * ここに配列を書き戻すと、また台帳が2つに割れる（「環の大きさ」を「環」の真下に置くために
 * カードの定義を移したのが実例 ＝ 並べ替えは定義順でできる）。
 *
 * 「カードに出てこない行は表に出さない」性質は renderMatrix 側（`used`）が持っている。
 */
const NARROW_ROWS = NARROW_ROW_ORDER;
...
const NW = {
    ...
    /**
     * ★★ エステルを加水分解して、**できたカルボン酸とアルコールの組**を返す（M12・v1505）。
     *
     ...
     *
     * ⚠ **酸無水物は組を返さない**（v1592）。`findFunctionalGroups` が `ester` ではなく `anhydride` で
     *   返すので、下の filter に入ってこない。⚠ それまでは -CO-O-CO- が ester×2 に見えていたため、
     *   **無水酢酸が「酢酸＋酢酸」を“酸とアルコールの組”として2つ返し**、アルコールの側に置いた
     *   酢酸の CH3-CO- を読んで `hyd-alc-iodoform`（得られたアルコールがヨードホルム陽性）にまで
     *   当たっていた。酸無水物の加水分解で出るのはカルボン酸2つで、アルコールは出ない
     *   ＝ M12 のカードの文言がどれも成り立たない。
     * ⚠ **切っても1分子のまま（ラクトン）は組を返さない。**
    ...
    partsLabel(m) {
        ...
        // 酸無水物は `ALLOT_PARTS` に前から部品として載っている（不飽和度2・酸素3）のに、
        // 候補の内訳では -CO-O-CO- が「エステル×2」として拾われて『エステル』（環なら『ラクトン』）と
        // 出ていた（v1592 で `findFunctionalGroups` が別の型を返すようになった）。
        // ⚠ 環でもラクトンとは呼ばない（無水フタル酸は環状の酸無水物であってラクトンではない）
        if (g.includes('anhydride')) parts.push('酸無水物');
