...
(() => {
    ...
    let rxLedger = null;
    let rxSamples = null;
    // ②ファズが「新しい面」（🧪 瓶・🎯 課題・📖 資料）へどこまで届いたかの帳簿（v1515・下の注記）
    let faceLedger = null;
    ...
    //
    // mix=4（v1515）… **新しい面3つの枝**（`bottle`／`quest`／`refbook`）が入った版。
    //   ⚠ 既存の10種の割合も下げている（合計 1.0 を保つため）ので、
    //   **mix=3 の実行とは1つも並べられない**（作図の面に当たる回数そのものが減る）。
    // mix=5（v1515・同じ日）… ⚠ **mix=4 の配分は作図の面を弱めすぎていた**（下の `quest` の注記）。
    //   `quest`／`refbook` を 1/3 に下げ、`place`／`module` へ戻した。
    //   ★ **割合を変えたら番号を上げる**という規約どおりに上げてある ——
    //   mix=4 で回した実行（このレーンの実測だけ）と mix=5 の実行を並べないため。
    const OP_MIX_ID = 5;
    const OP_MIX = [
        ['place', 0.29],    // 原子配置
        ['module', 0.13],   // モジュール配置
        ...
        ['bottle', 0.06],   // 🧪 実験パレットの瓶を押す（v1515）
        /* ⚠⚠ **`quest` と `refbook` の割合は「キャンバスを空にする回数」で決まっている**（v1515）。
         *   `startQuest` は `placeQuestStart` を通るので**押すたびにキャンバスが空になり**、
         *   資料の例題も `loadStage` で作図をやり直させる。
         *   ★ 実測（同じ種200個 × 80操作の A/B。`?nofaces=1` が否定対照）:
         *     quest 0.03・refbook 0.02 だと1反復で**平均 2.4 回**キャンバスが空になり、
         *     **ファズの失敗率が 20.0%（CI 15.0〜26.1）→ 4.0%（CI 2.0〜7.7）まで落ちた**
         *     ＝ 区間が重ならない ＝ **作図の面を見張る力が 5 倍 弱くなっていた**
         *     （分子が育つ前に消えるので、重なりが起きるところまで行かない）。
         *   ★ 1/3 に下げて 0.8 回/反復にした。⚠ **面への到達はこれでも足りる**
         *     （200反復で課題を始めた回数 504 → 160 でも、12/12 問すべて達成に届く）。
         *   ⚠ **上げ直すときは、失敗率も一緒に測ること。** 到達だけを見て上げると、
         *     監査がいちばん多く見つけている「作図の重なり」が黙って減る。 */
        ['quest', 0.01],    // 🎯 実験モードの課題を1問はじめる（v1515）
        ['refbook', 0.01],  // 📖 資料のページを1枚ひらく（v1515）
        ['undo', 0.05],
        ['redo', 0.03]
    ];
    ...
    const PAIR_SAMPLES = {
        ...
        /* ★ ジアゾカップリング（I-4）。⚠ **この行も `FZ1`・`FZ2` が自分で見つけて名指しした**
         *   （FZ1「表がずれている」・FZ2「題材を作れなかった反応が1本ある（diazo_coupling）」）
         *   ＝ 二重持ちの見張りが2回目も効いた。 */
        diazo_coupling: ['塩化ベンゼンジアゾニウム', 'ナトリウムフェノキシド'],
        /* ★ 共重合（v1541）。⚠ 2種類以上の単量体が要るので走査では拾えない。
         *   ⚠⚠ tests.js の `CV_PAIR_SAMPLES` と同じ中身にすること（`FZ1` が見張る）。 */
        copolymerization: ['スチレン', '1,3-ブタジエン', 'スチレン', '1,3-ブタジエン'],
        /* ★ アセチレンの三量化（v1541）。⚠ ちょうど3分子のときだけ通るので走査では拾えない。 */
        alkyne_trimerization: ['アセチレン（エチン）', 'アセチレン（エチン）', 'アセチレン（エチン）'],
        /* ★ ナトリウムフェノキシド ＋ ヨードメタン → アニソール（v1541）。⚠ 相手が要るので走査では拾えない。 */
        williamson_ether: ['ナトリウムフェノキシド（フェノールのナトリウム塩）', 'ヨードメタン（ヨウ化メチル）']
        /* ⚠⚠ **`amine_liberate_naoh` はここから外れた**（v1510 で「層の印が要る唯一の行」
         *   として入り、電荷の段〈I-3 の続き〉で不要になった）。detect が印ではなく
         *   **図そのもの**（アンモニウム塩の N）を見るようになったので、
         *   **登録済みのアニリン塩酸塩を呼び出すだけで題材になる**
         *   ＝ ライブラリの走査が自分で拾う（tests.js の `CV_PAIR_SAMPLES` と対で外した）。 */
    };
    ...
    /* ===== ★★ ファズを「新しい面」へ届かせる（v1515・DESIGN_review_pack3.md §1／発注書 B ②） =====
     *
    ...
    async function fuzzOnce(W, D, g, seed, opsCount, errBox, onOp) {
        ...
        errBox.length = 0;
        /* ★ 新しい面の後片付け（v1515）。⚠ **反復を独立させる**のは分子と履歴だけでは足りない ——
            ...
            try {
                ...
                if (kind === 'react') {
                    ...
                    if (!FACES_ENABLED || !rid) {
                         ...
                         *     ⛔ 分子モーダルの節（`#mm-reagent-note`）は **v1522 で消えた**
    ...
    /* 到達の数を画面に出す（★ **緑のときも出す**）。反応の面（v1502）と新しい面（v1515）を
    ...
    function buildSummary() {
        ...
        return {
            ...
            /* ★★ **新しい面へどこまで届いたか**（v1515・DESIGN_review_pack3.md §1）。
    ...
    /* ⚠ 再生では**新しい面の帳簿も要る**（v1515）。無いと `bottle`／`quest`／`refbook` の枝が
