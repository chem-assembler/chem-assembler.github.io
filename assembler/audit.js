...
(() => {
    ...
    // mix=5（vNNNN・同じ日）… ⚠ **mix=4 の配分は作図の面を弱めすぎていた**（下の `quest` の注記）。
    ...
    const OP_MIX = [
        ...
        /* ⚠⚠ **`quest` と `refbook` の割合は「キャンバスを空にする回数」で決まっている**（vNNNN）。
        ...
        ['quest', 0.01],    // 🎯 実験モードの課題を1問はじめる（vNNNN）
        ['refbook', 0.01],  // 📖 資料のページを1枚ひらく（vNNNN）
        ['undo', 0.05],
        ['redo', 0.03]
    ];
