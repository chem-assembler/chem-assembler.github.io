 ...
 * - ⚠ **「誤答肢どうしが同じことを言っている」型も、同じ理由で絞れなかった。**
 *   org.poly.vinylon-acetal の重複した誤答肢（v72 で修正）を捕まえる検査を書いたが、
 *   **狙ったものは鳴らず、鳴った8件のうち6件が正当な肢**だった。理由がはっきりしている ――
 *   選択肢は「Aである」「Bである」と**同じ文型で別の候補を並べる**のが正しい作り方なので、
 *   文字の重なりが高いのは**むしろ普通**。「エタノールである」と「ビニルアルコールである」は
 *   よく似ているが、別の物質を挙げた立派な肢である。
 * - → **ここから引ける教訓**: 肢や補足の**良し悪し**は文字の一致では測れない。
 *   この道具が見られるのは「**書いてはいけない語が入っているか**」までで、
 *   「**言っていることが妥当か**」は読む仕事。規則を足す前に、まず壊したデータで鳴るか試すこと。
  ...
  },
  {
    ...
    hit: function (p) {
      ...
      opts(p).forEach(function (o) {
        ...
        // ⚠ 「非常に」「異常に」「正常に」の中の「常に」は絶対語ではない（2026-09-23 qa-thick-basic 便の報告）
        var m = o.text.replace(/[非異正]常に/g, '').match(/(必ず|すべての|全ての|常に|決して|例外なく|いかなる)/);
