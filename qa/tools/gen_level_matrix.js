...
out.push('| 入試 | ① 解答になった回数（未 ＝ 判定していない）／② 手筋として使われた問題数 |');
...
out.push('');
out.push('| コード | 元 | 上書き | 日付 | 理由 | 記録 | 目安 §3-2 | 目安 §7-2 |');
out.push('|---|--:|--:|---|---|---|--:|---|');
ov.forEach((r) => {
  out.push('| `' + r.code + '` | ' + r.override.from + ' | **' + r.override.lv + '** | ' + r.override.date + ' | ' +
    cell(r.override.reason) + ' | ' + cell(r.override.ref || '') + ' | ' + ruleSemText(r) + ' | ' + ruleTabText(r) + ' |');
});
out.push('');
...
Q.units.forEach((u) => {
  ...
  out.push('');
  out.push('| コード | 群 | 現在 | 教科書 | セミナー | 入試 | 目安 §3-2 | 目安 §7-2 | 上書き | 最終 |');
  out.push('|---|---|--:|---|---|---|--:|---|---|--:|');
  rs.forEach((r) => {
    out.push('| `' + r.code + '` | ' + cell(groupOf[r.code]) + ' | ' + r.lv + ' | ' + tbText(r) + ' | ' + semText(r) + ' | ' +
      examText(r) + ' | ' + ruleSemText(r) + ' | ' + ruleTabText(r) + ' | ' + ovText(r) + ' | ' + r.final + ' |');
  });
  out.push('');
});
out.push('---');
out.push('');
out.push('生成元: `qa/questions.json`（' + rows.length + '件）・`qa/data/level_matrix.jsonl` の override 欄・セミナーの材料（リポジトリの外） ／ 生成器: `qa/tools/gen_level_matrix.js`');
out.push('');
...
const summary = rows.length + '件・上書き ' + ov.length + '件・上書きの記録なしの食い違い ' + loose.length + '件';
