...
| `oxidant` | 1 | `org.carbonyl.ketone-no-reduce` |
...
| `br2_water` | 1 | `org.aro.substitution-first` |
...
| `org.alcohol.na` | alcohol | 2 | ナトリウムとの反応は reactor の31種に無い。加えて水素の発生（気体）も assembler が表現しないので、反応が起きたこと自体が見えない |
...
| `org.aroN.aniline-base` | aroN | 2 | 塩基性の強さと塩酸塩の生成に対応する反応が reactor に無い（酸塩基反応そのものが未実装） |
...
| `org.bio.amino-acid-amphoteric` | bio | 2 | 双性イオンの電荷（−COO⁻ と −NH₃⁺）を assembler は表せず、強酸・強塩基と塩をつくる反応も reactor に無い |
...
| `org.carbonyl.acidity` | carbonyl | 2 | 弱酸性という液性、電離、塩基と中和して塩になる過程はいずれも reactor に無く、assembler はイオンや水溶液を扱わない |
| `org.carbonyl.salt-strong-acid` | carbonyl | 3 | カルボン酸の塩と強酸の反応（弱酸の遊離）は reactor に無く、塩やイオンを assembler は描かない |
| `org.carbonyl.vs-carbonic` | carbonyl | 3 | 酸の強弱の比較と、弱酸の遊離による二酸化炭素の発生（気体）は assembler に無い。塩の関与する反応も reactor の31種に含まれない |
...
| `org.phenol.na-h2` | phenol | 2 | 金属ナトリウムとの反応と水素の発生に対応する試薬が reactor の31種に無い |
| `org.phenol.naoh` | phenol | 2 | 中和して塩（ナトリウムフェノキシド）になる変化に対応する試薬が reactor の31種に無く、塩が水によく溶けることも表現できない<br><sub>ライブラリに「ナトリウムフェノキシド（フェノールのナトリウム塩）」があるので塩の形だけなら summon できる。反応として見せられないので採らなかった</sub> |
| `org.phenol.phenoxide-co2` | phenol | 3 | 弱酸の遊離は水溶液中の酸塩基平衡の話で、reactor には酸塩基反応そのものが無い |
