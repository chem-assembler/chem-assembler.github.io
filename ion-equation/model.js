...
function hydrolysisRule(stage) {
  return ((stage && stage.rules) || []).find((r) => r.kind === "hydrolysis") || null;
}
...
const STAGES = [
     ...
     クリアは「加水分解が1組でも起きて液性がかたよったこと」＋係数（DESIGN_ionic_two_step.md §6）。 */
  {
    ...
    // ビーカーへの模範投入数。per=5 なので5個入れて初めて1個ぶんの加水分解が起こる
    // （右辺のぶんは式の側の話なので、投入には使われない）
    answer: [5, 1, 1],
     ...
     画面の中で H⁺ が2通りの姿を持つ。教科書がそう書くことは doneNote で伝える。 */
  {
    ...
    products: ["NH3"],
    // 酢酸ナトリウムと同じく**ビーカーへの模範投入数**（式の係数ではない）。
    // 式は noMolecular なので必ず ionic 側が使われる
    answer: [5, 1],
