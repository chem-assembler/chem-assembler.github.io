...
(function (global) {
  ...
  var THERMO = [
    // 用語は新課程の ΔH 流儀にそろえる（レビュー S-5）。「生成熱」は発熱を正とする
    // 旧課程の量（CO₂ は＋394）なので、符号つきの −394 に添える語は「生成エンタルピー」。
    // 炭素の単体も同素体を明示して「C(黒鉛)」と書く（h6 が −394/−396 の違いを教えている）。
    // --- ①逆向きにたどると符号が反転する ---
    { id: 'h1', sig: 3, steps: {},
      levels: [{ key: 'a', label: 'C(黒鉛) ＋ O₂(気)' },
               { key: 'b', label: 'CO₂(気)' }],
      given: [{ from: 'a', to: 'b', dh: '-394', eq: 'C(黒鉛) ＋ O₂(気) → CO₂(気)' }],
      asked: { from: 'b', to: 'a', eq: 'CO₂(気) → C(黒鉛) ＋ O₂(気)' },
      note: '<b>逆向きにたどると符号が反転する</b>' },
    // --- ②ヘスの法則（一酸化炭素の生成エンタルピーは直接測れない） ---
    { id: 'h2', sig: 3, steps: { levels: true },
      ...
      asked: { from: 'a', to: 'b', eq: 'C(黒鉛) ＋ ½O₂(気) → CO(気)' },
      note: '<b>どの経路を通っても高さの差は同じ</b>（ヘスの法則）' },
    // --- ③生成エンタルピーから燃焼エンタルピーを出す（単体をいちばん上に置く） ---
    { id: 'h3', sig: 3, steps: { levels: true },
      levels: [{ key: 'a', label: 'C(黒鉛) ＋ 2H₂(気) ＋ 2O₂(気)' },
               { key: 'b', label: 'CH₄(気) ＋ 2O₂(気)' },
               { key: 'c', label: 'CO₂(気) ＋ 2H₂O(液)' }],
      given: [{ from: 'a', to: 'b', dh: '-75', eq: 'C(黒鉛) ＋ 2H₂(気) → CH₄(気)' },
              { from: 'a', to: 'c', dh: '-966',
                eq: 'CO₂ と 2H₂O の生成エンタルピーの和（−394 ＋ 2×(−286)）' }],
      asked: { from: 'b', to: 'c', eq: 'CH₄(気) ＋ 2O₂(気) → CO₂(気) ＋ 2H₂O(液)' },
      note: '単体をいちばん上の準位に置くと、生成エンタルピーがそのまま高さになる' },
    // --- ④結合エネルギー（原子をいちばん上に置く） ---
