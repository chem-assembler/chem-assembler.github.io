...
(function () {
  ...
  // 表示が値を書き切れないときは「…」（レビュー軽微・q12）。
  // 6.0×10²³/18 は浮動小数の上では整数なので「整数倍」と判定され、
  // 2桁に黙って丸めた「×3.3×10²²」がちょうどの倍率に見えていた
  ok('書き切れる数には … を付けない', M.numText(6e23) === '6.0×10²³');
  ok('書き切れない数は … を付ける（6.0×10²³/18）',
    M.numText(6.0e23 / 18) === '3.3…×10²²');
  ok('q12 の よこ倍率の表示は ×3.3…×10²²',
    M.factorText(M.factorH(byId('q12'))) === '×3.3…×10²²');
