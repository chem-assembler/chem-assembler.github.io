 ...
 */
/**
 * `--nocredit` で焼き込みを止める（2026-08-01 追加）。
 * **TikTok が「オリジナルではないコンテンツ」の警告を出す**ため、原因の切り分けに使う。
 * 右下に薄く小さい白文字という見た目が、機械にはウォーターマークと区別できないらしい。
 * **クレジット表記の義務はキャプション側（meta の credits が投稿文に出る）で満たせる**ので、
 * 焼き込みを外しても VOICEVOX の要件は満たしたままになる。
 */
const credit = ARGS.nocredit ? '' :
    (ARGS.credit || (ARGS.meta ? (JSON.parse(readFileSync(ARGS.meta, 'utf8')).credits || []).join(' / ') : ''));
if (credit) {
...
if (ARGS.meta) {
    ...
    const seriesCheck = (platform) => {
        ...
        return {
            ...
            // **TikTok のプレイリストはフォロワー1万人以上が条件**（2026-08-01 に判明。しかも作成はアプリのみ）。
            // 到達するまで指示として成立しないので、代わりに使える導線を出す＝
            // **できないことをチェックリストに残さない**（守れない項目が並ぶとリスト全体が読み飛ばされる）
            tiktok: [`プレイリスト「${m.series}」に追加する（**フォロワー1万人以上のときだけ**。未達ならこの行は飛ばす）`,
                     `プレイリストが使えないうちは、代わりに #${m.series} と、プロフィールの固定3枠でつなぐ`],
            instagram: [`シリーズの起点になる回はプロフィールに固定する（固定は3件まで）`],
            // **動画の中で「次回」と言った回だけ**繋ぐ（2026-08-01 に方針を絞った）。
            // 毎回やる指示にすると手間のわりに価値が薄く、リスト全体が読み飛ばされる。
            // 手順は「単独でポスト → 自分のそのポストに返信で前作URL」＝**返信は別投稿なので
            // 本文の280字を圧迫しない**（ここを誤解して本文に足すと入りきらない）
            x: [`**前作が「次回」と予告していた回なら**、前作のURLを自己返信で貼る（本文には足さない）`],
        }[platform] || [];
    };
    /**
