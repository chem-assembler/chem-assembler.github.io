 ...
 *   node tools/quiz-size-census.js --dropped        … **いま実際に外れているもの**（v1434 の上限）
 ...
 *
 * **2026-08-21・上限が入った（v1434）。** ユーザーの決定は
 ...
 *
 * ⚠ **本体は `assembler/quiz.js` の `longestChainOutsideRing`**（v1434 で上限の物差しに
...
if (has('dropped')) {
    /* ⚠ **v1434 で実際に入った上限（QUIZ_CHAIN_MAX）で、何が出題プールから外れているか。**
    ...
    /* 上限を引いたら何が外れるか（**どこで切るかを決めるための材料**。
       ...
       v1434 で 鎖≦10 が入った後も、線を引き直したくなったときのために残してある）。
       2つの物差しを並べる:
         ・重原子数            … 図全体の大きさ
