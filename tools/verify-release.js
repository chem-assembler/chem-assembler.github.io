...
targets.sort().forEach(dir => {
    ...
    /* 3.3b. `?v=` の中身が数字でない（＝プレースホルダの置き忘れ）
     *
     * ⚠⚠ **これは 2026-09-07 まで丸ごと死角だった。** ASSET_RE は `?v=(\d+)` しか拾わないので、
     * レーンが書く `blocks.js?v=vNNNN` は
     *   ・url が `blocks.js?v=vNNNN` として丸ごと吸われる
     *   ・`path.extname()` が `.js?v=vNNNN` になり、3.4 の拡張子ふるいで**そのまま捨てられる**
     * ＝ 「資産の実在」も「?v= の付け忘れ」も**両方すり抜ける**。
     * 統合レーンが実番号への差し替えを1か所忘れると、そのファイルは
     * **永久に同じクエリで配られる**（＝直しても古い実体がキャッシュから返る）。
     *
     * ★ 直しは「数字以外の `?v=` を見つけたら赤」。プレースホルダの綴り（vNNNN・NNNN・
     * TODO など）を数え上げるのではなく、**数字でないものを全部**捕まえる。 */
    files.forEach(f => {
        // ⚠ `?v=41&amp;r=r14` のように後ろに別の引数が続くことがあるので、`&` で切る
        [...f.text.matchAll(/(?:src|href)="([^"]*?)\?v=([^"&]*)/g)].forEach(m => {
            const [, url, v] = m;
            if (isExternal(url)) return;
            if (/^\d+$/.test(v)) return;
            problems.push(`${f.rel}: ${url}?v=${v} の版が数字ではありません`
                + `（プレースホルダの置き忘れ。このファイルは永久に同じクエリで配られます）`);
        });
    });
