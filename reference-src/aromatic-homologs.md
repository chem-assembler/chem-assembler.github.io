...
:::callout
tone: note
text: 実際の操作は**中性〜塩基性**で行います。そのため KMnO₄ は **MnO₂**（黒褐色の沈殿・Mn の酸化数は +7 → +4）になり、できた安息香酸は**安息香酸カリウム**（塩）のまま溶けています。最後に**強酸を加えて**はじめて安息香酸が遊離します。入試では、Mn の酸化数の変化や「最後に強酸を加える理由」が問われます。
:::

:::link
to: half-reaction-condition
text: 液性によって、その液性で実際に存在する形（MnO₂・カルボン酸の塩）で書く
:::
