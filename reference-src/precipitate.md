...
//app: ion-equation/ rxn=s4 「硝酸銀と塩化ナトリウム（沈殿）」
...
//撮影: url=/muki/snake.html sel=.dict-section:nth(2)（「全沈殿リスト」）を y=170〜900 で切り取り 状態=「イオン図鑑を開く」を押しただけ（抽選なし）・viewport 1200×2600・deviceScaleFactor 1・256色に減色
...
鉄(III)イオンの赤褐色の沈殿は、Fe(OH)₃ と書くことが多いのですが、実際には決まった組成をもたない沈殿で、FeO(OH) と書く教科書もあります。どちらで書いても、**「赤褐色の沈殿」**と答えられれば十分です。

//要確認: 化学新研究2015・総合的研究化学 Fe³⁺ の水酸化物の沈殿を Fe(OH)₃ と FeO(OH) のどちらで書いているか（教科書は FeO(OH)、muki も FeO(OH)）
...
//⇄ 本文中の H₂S ⇄ 2H⁺ ＋ S²⁻ は文の中に書いた。:::reaction に arrow: が入ったら式にしてもよい
...
- 水酸化物の色（Cu(OH)₂ 青白・Ag₂O 褐・Fe(OH)₂ 緑白・水酸化鉄(III) 赤褐・ほかは白）
